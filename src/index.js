...
const resolvers = {
  Query: {
    ...
    myPlans: async (_, __, { db, user }) => {
      ...
      const plans = await db.collection('Plans').find({ userIds: { $in: [ObjectId(user._id)] } }).toArray()
      return plans
    },
    getPlansByUserIds: async (_, { users }, { db, user }) => {
      if (!user) {
        throw new Error('Invalid Credentials!');
      }
      ...
      const plans = await db.collection('Plans').find({ userIds: { $in: [user._id].concat(...u) } }).toArray()
      return plans
    },
